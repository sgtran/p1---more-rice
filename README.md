...
| [Ticket 4](https://github.com/sgtran/p1---rice/issues/15): Add Easter eggs -Alex Hu| Designed ideas for easter egg: instead of a simple, unoriginal button, I have decided to add in an automatic new tab link to journals, old frqs etc when a particular username is entered on the main menu. Implementation is still in the process as this week we were mainly learning about Spring and server hosting basics. Will be achieved by having a Spring method/listener in the constructor for setting up the playfield that checks what the username is.|
